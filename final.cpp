...
bool     Frozen;                     //freeze current  status


// global variables;
float Time;
float BladeAngle;
float Move, planemove;
// function prototypes:

void    Animate();
void    Display();
void    DoChangeViewMenu(int);
void    DoAxesMenu(int);
...
Display()
{
	...
	glPushMatrix();
	glTranslatef(3 + planemove / 2, planemove / 5, (planemove) / 4);
	glTranslatef(-20., 4, 5);
	...
	if (planemove <= 35)
	{
		glTranslatef(0., 5, -(planemove));
		glTranslatef(0., 5, 50);
		glCallList(BoxList);
	}
	else
	{
		glTranslatef(0., 7.9, 15);
	...
	if (planemove > 35)
	{
		...
		glTranslatef(-planemove / 5, -planemove / 5, 0);
		glTranslatef(-2., 10, 15);
		glCallList(Fraglist);
	...
	if (planemove <= 35)
	{
		glTranslatef(0., 6.3, -2 + (-(planemove)));
		glTranslatef(0., 6.3, 50);
		glRotatef(BladeAngle / 3, 0., 6.3, 0.);
		//glRotatef(BladeAngle, 0., 2.9, 0.);
		glRotated(90, 1., 0., 0.);
	...
	if (planemove <= 35)
	{
		glTranslatef(.3, 5.7, -9 + (-(planemove)));
		glTranslatef(.3, 5.7, 67.1);
		glRotated(90, 0., 0., 1.);
		glRotatef(BladeAngle, 0.3, 5.7, 0.);
		//glRotatef(BladeAngle, .5, 0., 0.);
		glRotated(90, 0., 1., 0.);
...
InitMenus()
{
	...
	int changeviewmenu = glutCreateMenu(DoChangeViewMenu);
	...
	glutAddSubMenu("ChangeView", changeviewmenu);
...
InitLists()
{
	...
	Fraglist = glGenLists(1);
	glNewList(Fraglist, GL_COMPILE);
	glPushMatrix();
	glScalef(4., 4., 4.);
	glColor3f(1., 1., 1.);
	...
	glPushMatrix();
	glColor3f(1.0, 1.0, 0.);
	glutSolidSphere(1.0, 10., 15.);
	glPopMatrix();
	glEndList();


	Antilist = glGenLists(1);
	glNewList(Antilist, GL_COMPILE);
	glPushMatrix();
	glColor3f(0.0, 1.0, 1.0);
	LoadObjFile("anti.obj");
	glPopMatrix();
	glEndList();
	...
	glPushMatrix();
	unsigned char* Texture;
	Texture = BmpToTexture("doge.bmp", &width, &height);
	...
	Texture2 = BmpToTexture("background.bmp", &width, &height);
	//glGenTextures(1, &texName);
	//glBindTexture(GL_TEXTURE_2D, texName);
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
	...
	/*******************Blade_TOP*********************/
		...
		//    width BLADE_WIDTH centered at (0.,0.,0.) in the XY plane
												...
												//Scale the blade to be the right size.
												//Rotate the blade to be in its proper orientation.
												//Rotate the blade by its current spinning rotation angle.
												//Translate the blade to where it needs to be on the helicopter.
												//If you are in the Outside view, apply the Xrot, Yrot, and Scale transformations as before.
												//Assign a good eye, look, and up position with gluLookAt.
	Bladelist1 = glGenLists(1);
	glNewList(Bladelist1, GL_COMPILE);
	glPushMatrix();

	//glRotated(BladeAngle, 0., 2.9, 0.);
	glScalef(5., 5., 5.);
	glColor3f(1., 1., 1.);
	glBegin(GL_TRIANGLES);
	glVertex2f(BLADE_RADIUS, BLADE_WIDTH / 2.);
	...
	Texture1 = BmpToTexture("worldtex.bmp", &width, &height);
	//glGenTextures(1, &texName);
	//glBindTexture(GL_TEXTURE_2D, texName);
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
